...
def load_data_kobe(data):
    """
    Leitura dos dados iniciais disponibilizados pelo Kaggle e filtro 
    das informações que serão utilizadas na modelagem.       
    """
...
def generate_base_inference(data):
    """
    Geração dos dados que serão utilizados para inferência do modelo.
    """
...
def conform_data_kobe(data):
    """
    Conformação dos dados para utilização na modelagem.
    """
    data = data.dropna()
    return data   
...
def train_test_split_data_kobe(data, n_test_size, n_random_state):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=n_test_size, 
                                                        random_state = n_random_state, stratify=y)
...
def metrics_preparacao_dados(data, data_train, data_test):
    """
    Registro no MLFlow dos tamanhos das bases de treino e teste.
    """
